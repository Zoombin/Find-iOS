...
- (void)setupView
{
	...
	_editSampleButton = [UIButton buttonWithType:UIButtonTypeCustom];
	_editSampleButton.frame = CGRectMake(0, 0, 100, 30);
	[_editSampleButton setTitle:@"edit photo" forState:UIControlStateNormal];
	_editSampleButton.backgroundColor = [UIColor redColor];
	[borderView addSubview:_editSampleButton];

	_choosePhotoButton = [UIButton buttonWithType:UIButtonTypeCustom];
	_choosePhotoButton.frame = CGRectMake(0, CGRectGetMaxY(_editSampleButton.frame), 100, 30);
	_choosePhotoButton.backgroundColor = [UIColor blueColor];
	[borderView addSubview:_choosePhotoButton];

//	_backgroundImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_choosePhotoButton.frame), self.view.bounds.size.width, self.view.bounds.size.height - CGRectGetMaxY(_choosePhotoButton.frame))];
...
- (void) layoutImageViews
{
    ...
    CGFloat height = CGRectGetMinY(choosePhotoFrame) - 2.0f * kAFSDKDemoImageViewInset;
    CGFloat width = aspectRatio * height;
    CGRect imageFrame = CGRectMake((CGRectGetWidth(bounds) - width) / 2.0, kAFSDKDemoImageViewInset, width, height);
